using MethodOfLines, ModelingToolkit, OrdinaryDiffEq, DomainSets, Plots, Printf
...
anim = @animate for i in 1:length(sol[t])
    plot = surface(sol[ϵ], sol[x], transpose(solu[i, :, :]), xlabel="Energy", ylabel="Position", zlabel="n", camera=(80, 50), color=reverse(cgrad(:RdYlBu_11)), clims=(zmin, zmax))
    zlims!(zmin, zmax)
    titleStr = Printf.format(Printf.Format("%.2e"),(i-1)/length(sol[t]) * maxTime)
