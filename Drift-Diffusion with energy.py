...
shouldUseBCs = True
taskType = "gridSearch" # Options: timeEvo, gridSearch
...
def calculatePDE(dt=dt, F = F, sigma=sigma, Lambda=Lambda):
    ...
    # Initial field
    state = pde.ScalarField.from_expression(grid, initialField + f"* heaviside( (x-{energyRange[0]})*(x-{energyRange[1]})*(y-{positionRange[0]})*(y-{positionRange[1]}), 0) ")
...
if taskType == "gridSearch":
    ...
    parameterData = [{"name": "dt", "range": [-1.7, 0], "numPoints": 6, "defaultValue": 1e-1, "isLog": True},
                     ...
                     {"name": "σ", "range": [0.02, 0.20], "numPoints": 6, "defaultValue": 0.13, "isLog": False},
                     {"name": "λ", "range": [-3, -6], "numPoints": 6, "defaultValue": -4, "isLog": True}]
    ...
    output = []
    # if numData == 2:
    #     fig = plt.figure(figsize=(3*parameterData[chosenDataIndices[1]]["numPoints"]+1,4*parameterData[chosenDataIndices[0]]["numPoints"]))
